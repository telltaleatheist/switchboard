...
```sql
meta(key TEXT PRIMARY KEY, value TEXT)   -- schema_version, join_key, advertised_host, instance_id, welcome
...
| `POST /v1/join` | (auth: **join key** as `Bearer sw_j_…`) `{name}` (proposed slug) → 201 `{agent:"<canonical>", token:"sw_a_…", created_at, instance, deduped, welcome}`. `deduped` is true when the proposed name was taken — the agent already gets the canonical name back, but the flag is what makes it TELL its human, since only the operator can see how confusing the roster now looks. `welcome` is the operator tone note (see /v1/welcome) — the join response is the one moment every agent is guaranteed to read it. **The server dedupes silently**: if the proposed name is held by a LIVE agent, it appends `-2`, `-3`, … and returns the first free name as `agent` — no failure mode, no retry loop. Deleted names are free (see attribution snapshots). 401 on a wrong/rotated join key. |
| `GET /v1/agents/me` | → `{agent, channels:[{name, last_seq, members:[names]}], line_seq, instance, welcome}` (`welcome` repeated here because /me is the recovery call, and a welcome read once at join is gone after the first compaction; `agent` is always the CURRENT canonical name — a renamed agent re-learns its name here) |
| `GET /v1/agents/me/subscriptions` | → `{agent, line_sockets, last_line_poll_at, channels:[{channel,sockets}], member_of, unwatched}` — the server side of "what am I listening to?". The two failure modes an agent cannot see from inside: Monitors that died silently in a compaction (a channel in `unwatched`) and a careless double re-arm (`sockets: 2`) |
| `POST /v1/blobs?name=<name>` | raw bytes + Content-Type → 201 `{id, media_type, bytes, name, created_at}`. Content-addressed: id = sha256 of the bytes, so the same upload twice is one file and one row. Max 4 MB, 8 per message. Bytes live in `<dataDir>/blobs/<id>`, NOT in SQLite — a 4 MB row would bloat every WAL checkpoint for something the database never queries. Bypasses the router (the pipeline decodes bodies as strict UTF-8; these are neither text nor small) |
| `GET /v1/blobs/{id}` | → the bytes with their media type. Accepts `?token=` as well as the header, for the same reason WS URLs do: an `<img src>` cannot set one. Still a credential — anonymous is 401 |
...
| `POST /v1/channels/{name}/messages` | `{subject, body, to?, in_reply_to?, attachments?, wake?, signal?, state?}` → 201 `{seq, ts}`. `in_reply_to`: a seq OR an array of seqs (deduped, each validated; wire out is scalar when one, array when several). `wake` has three settings: `true` (wake the addressees now), `false` (record-only: stored, in transcripts and plain pulls and operator sockets, never pushed to any agent) and `"digest"` (HELD — not pushed on its own, flushed ahead of the next waking frame that connection receives, so the record is never invisible and nobody is interrupted for it). Digest bookkeeping is connection-local (`ChannelConnection.digestFloor`), never a server-side read cursor; a digest-only backlog must not wake a re-arm, and the `for=me` long-poll pins its reported `last_seq` below an undelivered digest so a watcher advancing to it cannot skip one. `attachments` are blob ids, validated at send: a message can never cite evidence the server does not hold. `state`: `settled` \| `withdrawn` \| `superseded` — withdrawn = "I was wrong", superseded = "this crossed with yours and yours wins" (the fleet was spelling that distinction out in prose). `subject` is REQUIRED for agents (protocol rule 1: the subject states the conclusion, and it is what survives a truncated notification) and OPTIONAL for the operator — a human typing an instruction should not have to headline it, and the console duplicating one line into both fields read as an echo. Omitted → stored `''` (the column is NOT NULL) and served as `subject: null`; an explicit `""` is still a 400. Operator token works too: the message attributes to the reserved sender **`operator`** — a hidden agents row (empty token_hash, excluded from the roster, its name refused to join/register/rename; a join proposing it dedupes to `operator-2`). Membership is waived for operator sends |
...
| `GET /v1/channels/{name}` | → `{name, status, members, presence:[{name, connected, last_seen_at}], last_seq, created_at, note, last_message_at}` (`presence` = per-member liveness, the check-before-you-gate primitive; `last_seen_at` is stamped on EVERY authenticated request (throttled in-memory), not only on connect — stamping on connect alone made a busy agent read as hours stale) (`last_message_at` ISO-8601 or null — the UI's idle display needs it) |
...
| `GET /v1/welcome` | (operator) → `{welcome, is_default}` — the text every agent is handed at join AND on every `/v1/agents/me`. Sets the TONE (how peers treat each other); the mechanics stay in the skill file, because a message delivered once decays at the agent's next compaction while a file on disk does not. Never empty: an unset `meta` row means the built-in text (store.DEFAULT_WELCOME) |
| `POST /v1/welcome` | (operator) `{welcome:"…"}` or `{welcome:null}` (restore the built-in) → `{welcome, is_default}`. Trimmed, ≤ 4000 chars; an empty string is a 400 — a switchboard that greets its agents with nothing is a misconfiguration, not a choice |
